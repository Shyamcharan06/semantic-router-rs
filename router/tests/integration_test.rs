...
use semantic_router::classifier::Classifier;
...
async fn spawn_router_server(backend_addr: SocketAddr, security: SecurityConfig) -> SocketAddr {
    ...
    let router = RoutingStrategy::Similarity(SemanticRouter::new(category_indexes, 0.35));
...
/// Exercises the actual weights file shipped in the repo (generated by
/// eval/train_classifier.py from config/routes.example.yaml's category
/// examples), not a synthetic fixture -- if this fails, the checked-in
...
async fn trained_classifier_routes_correctly_using_real_shipped_weights() {
    ...
    let backend_addr = spawn_mock_backend().await;
    let classifier = Classifier::load(&classifier_path).expect("failed to load classifier.json");
    let embedder = shared_embedder().await;
    ...
    let resp = client
        ...
        .query(&[("q", "Write a Python function to reverse a linked list")])
        .send()
        .await
        .unwrap();
    let body: Value = resp.json().await.unwrap();
