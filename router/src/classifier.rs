...
impl Classifier {
    pub fn load(path: impl AsRef<Path>) -> Result<Self> {
        let raw = std::fs::read_to_string(path.as_ref()).with_context(|| {
            format!("failed to read classifier weights file {:?}", path.as_ref())
        })?;
        ...
        if let Some(row) = parsed.weights.first()
            && parsed.weights.iter().any(|w| w.len() != row.len())
        {
    ...
    pub fn predict(&self, embedding: &[f32]) -> (String, f32) {
        ...
        let (best_idx, best_logit_exp) = exp.iter().enumerate().fold(
            (0usize, f32::MIN),
...
mod tests {
    ...
    fn ambiguous_input_gives_lower_confidence() {
        ...
        assert!(
            prob < 0.9,
            "expected lower confidence for ambiguous input, got {prob}"
        );
    }
